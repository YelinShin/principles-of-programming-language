...
(define find_max_char
  (lambda (l)
    (find_n (list_num_letter p) (apply max (list_num_letter p))))) ; it gives index number of the max char
...
(define Code-Breaker
  (lambda (d decoder)
    (if (null? d) '()
        (append (map decoder (car d)) (Code-Breaker (cdr d) decoder))) ;use recursion to change the document to be correct
     ))
